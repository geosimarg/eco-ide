<script setup lang="ts">
  ...
  import { MenubarMenuItemShape } from '@/types/menubar_menu_item_shape';
  ...
  function isSeparator(item: MenubarMenuItemShape): item is { type: 'separator' } {
    return item.type === 'separator';
  }

  const menus: { id: string; label: () => string; items: MenubarMenuItemShape[] }[] = [
    {
      id: 'file',
      ...
      <div v-if="activeMenu === menu.id" class="dropdown">
        <template v-for="(item, index) in menu.items" :key="index">
          ...
          <div v-else class="dropdown-item"
            :class="{ disabled: typeof item.disabled === 'function' ? item.disabled() : item.disabled, 'has-submenu': item.submenu }"
            ...
            <div v-if="item.submenu" class="submenu">
              <div v-for="sub in item.submenu" :key="sub.id" class="dropdown-item"
                :class="{ disabled: typeof sub.disabled === 'function' ? sub.disabled() : sub.disabled }"
