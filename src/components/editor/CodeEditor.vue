<script setup lang="ts">
  ...
  function createEditor() {
    ...
    let fileContent = props.file.content || '';

    if (!fileContent && props.file.path) {
      invoke<string>('read_file', { path: props.file.path })
        .then(content => {
        ...
        .catch(err => logger.error('Erro ao ler arquivo:', err));
    }
    ...
    scrollHandler = handleEditorScroll;
  ...
  function destroyEditor() {
    if (editorView && scrollHandler) {
      editorView.scrollDOM.removeEventListener('scroll', scrollHandler);
      scrollHandler = null;
    }
...
<template>
  <div class="code-editor-wrapper">
    ...
    <Minimap :content="file.content || ''" :scrollPercent="minimapScrollPercent" @scrollChange="handleMinimapScroll" />
  </div>
</template>
