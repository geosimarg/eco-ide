<script setup lang="ts">
  ...
  import { ContextMenuItem } from '@/interfaces/context_menu_item';
  import { EditorTab } from '@/interfaces/editor_tab';

  const props = defineProps<EditorTab>();
  ...
  const isDiff = computed(() => props.file.isDiff === true);

  const tabTitle = computed(() => {
    if (isDiff.value && props.file.diffLabel) {
      return `${props.file.name} (${props.file.diffLabel})`;
    }
...
<template>
  <div class="tab" :class="{ active, 'drag-over': isDragOver, pinned: file.pinned, 'diff-tab': isDiff }"
    draggable="true" @click="emit('select')" @dragstart="emit('dragStart', $event)" @dragover="onDragOver"
    @dragleave="onDragLeave" @drop="onDrop" @contextmenu="handleContextMenu" :title="tabTitle">
    <FileIcon :name="isDiff ? 'git-diff' : props.file.name" class="tab-icon" />
    <span class="tab-name no-select">{{ file.name }}<template v-if="isDiff && file.diffLabel"> ({{ file.diffLabel
        }})</template></span>
