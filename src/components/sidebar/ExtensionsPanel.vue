...
<template>
  <div class="extensions-panel">
      ...
      <div class="header-actions">
        <button class="btn-icon" @click="handleRefresh" :title="i18n.t('extensions.refresh')">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            <path d="M14 2a7 7 0 01-7 7 7 7 0 01-4.5-1.9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
            <path d="M2 14a7 7 0 017-7 7 7 0 014.5 1.9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
          </svg>
        </button>
        <button class="btn-add-folder" @click="extStore.browseExtensionsFolder">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            <path d="M1 4v7a1 1 0 001 1h10a1 1 0 001-1V5a1 1 0 00-1-1H7l-2-2H2a1 1 0 00-1 1z" stroke="currentColor" stroke-width="1.2"/>
    ...
    <div v-if="activeTab === 'marketplace'" class="category-filters">
      <button 
        class="category-chip"
        :class="{ active: !extStore.selectedCategory }"
        @click="extStore.selectedCategory = null"
      >
        All
      </button>
      <button 
        v-for="cat in extStore.categories" 
        :key="cat.id"
        class="category-chip"
        :class="{ active: extStore.selectedCategory === cat.id }"
        @click="extStore.selectedCategory = cat.id"
      >
    ...
    <template v-else>
      ...
      <div v-if="activeTab === 'marketplace'" class="extension-list">
        <div
          v-for="ext in extStore.filteredMarketplaceExtensions"
          :key="ext.id"
          ...
          <div class="ext-actions">
            <template v-if="ext.installed">
              ...
              <button 
                ...
                :title="i18n.t('extensions.uninstall')"
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <div v-else-if="activeTab === 'installed'" class="extension-list">
        <div
          v-for="ext in extStore.installedExtensions"
          :key="ext.id"
          class="extension-card"
        >
          <div class="ext-icon">
      ...
      <div v-else-if="activeTab === 'local'" class="extension-list">
        <div
          v-for="ext in extStore.filteredLocalExtensions"
          :key="ext.id"
          class="extension-card"
        >
